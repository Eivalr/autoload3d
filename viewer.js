...
const Viewer = (() => {
  ...
  let currentEq = null;
  let orbitState = {
    theta: Math.PI / 5, phi: Math.PI / 3.5, r: 0,
  ...
  function init(mountEl) {
    ...
    const W = mountEl.clientWidth || 800, H = mountEl.clientHeight || 480;
    ...
    setupOrbit(mountEl); setupTooltip(mountEl);
  ...
  function renderContainer(container, eq) {
    clearScene();
    currentContainer = container; currentEq = eq;
    const { L, W, H } = eq;
    ...
    // Floor — warm timber brown, clearly distinct
    const isDark = document.documentElement.getAttribute('data-theme') !== 'light';
    ...
    for (let px = -L/2; px <= L/2; px += 30) {
      const pts = [new THREE.Vector3(px, 2, -W/2), new THREE.Vector3(px, 2, W/2)];
      contGroup.add(new THREE.Line(new THREE.BufferGeometry().setFromPoints(pts), plankMat));
    }
    ...
    container.placed.forEach(p => {
      ...
      const mat = new THREE.MeshStandardMaterial({ color, transparent: true, opacity: isWireframe ? 0 : 0.85, wireframe: isWireframe, roughness: 0.65, metalness: 0.08 });
      const mesh = new THREE.Mesh(geo, mat);
      ...
      // Shipment ID label — top face
      const s1 = makeShipmentLabel(p.item.shipment, color);
      s1.position.set(0, p.ih/2 + 0.5, 0);
      s1.scale.set(Math.min(p.il, p.iw) * 0.9, Math.min(p.il, p.iw) * 0.45, 1);
      mesh.add(s1);

      // Shipment ID label — front face
      const s2 = makeShipmentLabel(p.item.shipment, color);
      s2.position.set(0, 0, p.iw/2 + 0.5);
      s2.scale.set(Math.min(p.il, p.ih) * 0.85, Math.min(p.il, p.ih) * 0.4, 1);
      mesh.add(s2);
  ...
  function addBoxContainer(group, L, W, H, color) {
    ...
    group.add(new THREE.Mesh(cGeo.clone(), new THREE.MeshStandardMaterial({ color: color||0x3a4060, transparent:true, opacity:0.04, side:THREE.BackSide })));
  }

  function makeShipmentLabel(shipmentId, boxColor) {
    const canvas = document.createElement('canvas');
    canvas.width = 256; canvas.height = 128;
    const ctx = canvas.getContext('2d');
    ctx.fillStyle = 'rgba(0,0,0,0.62)';
    roundRect(ctx, 4, 4, 248, 120, 12); ctx.fill();
    const r=(boxColor>>16)&0xff, g=(boxColor>>8)&0xff, b=boxColor&0xff;
    ctx.strokeStyle=`rgb(${r},${g},${b})`; ctx.lineWidth=5;
    roundRect(ctx,4,4,248,120,12); ctx.stroke();
    ctx.fillStyle='#ffffff'; ctx.font='bold 38px Arial,sans-serif';
    ctx.textAlign='center'; ctx.textBaseline='middle';
    ctx.fillText(shipmentId.length>10?shipmentId.substring(0,10)+'…':shipmentId, 128, 64);
  ...
  function renderLegend(container) {
    ...
    legend.innerHTML = shipments.map(sid=>`<div class="leg-item"><div class="leg-dot" style="background:${hexCSS(getColor(sid))}"></div>${sid}</div>`).join('');
  }

  function toggleWireframe() {
    isWireframe = !isWireframe;
    cargoMeshes.forEach(m => { if (m.material) { m.material.wireframe=isWireframe; m.material.opacity=isWireframe?1:0.85; } });
  }

  function toggleExplode() {
    isExploded = !isExploded;
    if (!currentEq) return;
    const factor = isExploded?1.4:1.0, cx=0, cy=currentEq.H/2, cz=0;
    explodeOrigins.forEach(({mesh,origin}) => mesh.position.set(cx+(origin.x-cx)*factor, cy+(origin.y-cy)*factor, cz+(origin.z-cz)*factor));
  }

  function resetCamera() { orbitState.theta=Math.PI/4; orbitState.phi=Math.PI/3.5; updateCamera(); }

  function setupOrbit(el) {
    orbitState.target = new THREE.Vector3(0,200,0); orbitState.r=2500;
    el.addEventListener('mousedown', e => { orbitState.dragging=true; orbitState.rightDrag=e.button===2; orbitState.lastMouse={x:e.clientX,y:e.clientY}; e.preventDefault(); });
    ...
    window.addEventListener('mousemove', e => {
      ...
      } else { orbitState.theta-=dx*0.006; orbitState.phi=Math.max(0.08,Math.min(Math.PI-0.08,orbitState.phi+dy*0.006)); }
      updateCamera();
    });
    window.addEventListener('mouseup', ()=>{ orbitState.dragging=false; });
    el.addEventListener('wheel', e=>{ orbitState.r=Math.max(100,Math.min(20000,orbitState.r+e.deltaY*2)); updateCamera(); e.preventDefault(); },{passive:false});
    let lastT=null;
    el.addEventListener('touchstart', e=>{ if(e.touches.length===1) lastT={x:e.touches[0].clientX,y:e.touches[0].clientY}; },{passive:false});
    el.addEventListener('touchmove', e=>{
      if(!lastT) return;
      const dx=e.touches[0].clientX-lastT.x, dy=e.touches[0].clientY-lastT.y;
      orbitState.theta-=dx*0.006; orbitState.phi=Math.max(0.08,Math.min(Math.PI-0.08,orbitState.phi+dy*0.006));
  ...
  function setupTooltip(el) {
    ...
    el.addEventListener('mousemove', e => {
      ...
      if (hits.length>0) {
        ...
        tip.innerHTML=`<strong>#${seq} — ${item.desc}</strong><br>Shipment: <em>${item.shipment}</em><br>${item.l}×${item.w}×${item.h} cm · ${item.weight} kg<br>Placed: ${placement.il}×${placement.iw}×${placement.ih} cm<br>CBM: ${cbm} m³ · Cat: ${item.category} · P${item.stackPriority}`;
  ...
  return { init, renderContainer, toggleWireframe, toggleExplode, resetCamera, exportPNG, getColor, hexCSS, clearScene };
})();
